...
# yay, multilib
pacman -S --needed less git base-devel && git clone https://aur.archlinux.org/yay-bin.git && cd yay-bin && makepkg -si
...
# korean
yay --noconfirm -S ttf-kopub ttf-jetbrains-mono-nerd ttf-d2coding

# dev
yay --noconfirm -S ghostty nvim ripgrep jq net-tools wl-clipboard fastfetch sysstat

# util
yay --noconfirm -S readest ytmdesktop-bin firefox google-chrome

# swayosd
sudo systemctl enable --now swayosd-libinput-backend

# waybar
ln -sf ~/dotfiles/arch/waybar ~/.config/waybar
sudo rm -f /etc/xdg/autostart/nm-applet.desktop

# zsh
yay -S zsh-autosuggestions zsh-syntax-highlighting oh-my-zsh-git
chsh -s $(which zsh)

sudo ln -s /usr/share/zsh/plugins/zsh-autosuggestions/ ${ZSH_CUSTOM:-~/.oh-my-zsh/custom}/plugins/zsh-autosuggestions
sudo ln -s /usr/share/zsh/plugins/zsh-syntax-highlighting/ ${ZSH_CUSTOM:-~/.oh-my-zsh/custom}/plugins/zsh-syntax-highlighting
